...
class AppearanceSettingsViewController: NSViewController {
    lazy var imageGridThumbnailSizeSlider: NSSlider = {
        let slider = NSSlider(value: 0, minValue: 50, maxValue: 500, target: self, action: #selector(action))
    ...
    lazy var imageGridSpacingSlider: NSSlider = {
        let slider = NSSlider(value: 0, minValue: 0, maxValue: 100, target: self, action: #selector(action))
